...
const clearWest = (lat, lon, tree) => {
  for (let x = lon - 1; x >= 0; x--) {
    if (trees[lat][x] >= tree) return false
  }
  return true
}
...
const clearNorth = (lat, lon, tree) => {
  for (let y = lat - 1; y >= 0; y--) {
    if (trees[y][lon] >= tree) return false
  }
  return true
}
...
const countWest = (lat, lon, tree) => {
  let viewRange = 0

  for (let x = lon - 1; x >= 0; x--) {
    viewRange++
    if (trees[lat][x] >= tree) break
  }
  return viewRange
}

const countEast = (lat, lon, tree) => {
  let viewRange = 0
...
const countNorth = (lat, lon, tree) => {
  let viewRange = 0

  for (let y = lat - 1; y >= 0; y--) {
    viewRange++
    if (trees[y][lon] >= tree) break
  }
  return viewRange
}

const countSouth = (lat, lon, tree) => {
  let viewRange = 0
...
for (let lat = 1; lat < rowsAmount - 1; lat++) {
  for (let lon = 1; lon < colsAmount - 1; lon++) {
    ...
    const rangeWest = countWest(lat, lon, tree)
    const rangeEast = countEast(lat, lon, tree)
    const rangeNorth = countNorth(lat, lon, tree)
    const rangeSouth = countSouth(lat, lon, tree)

    const treeRange = rangeWest * rangeEast * rangeNorth * rangeSouth

    if (treeRange > bestRange) bestRange = treeRange
  }
}
