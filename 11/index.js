...
const run2 = (rounds) => {
  ...
  const lcm = divisors.reduce((a, b) => a * b) //? Least Common Multiple

  for (let round = 1; round <= rounds; round++) {
    for (let monkey of monkeys) {
      while (monkey.items.length > 0) {
        let item = monkey.items[0]
        //? If item is greater than lcm, then
        //? removing the lcm from the item will not
        //? change the outcome of item % any of the lcm factors.
